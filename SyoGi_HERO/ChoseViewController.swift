...
class ChoseViewController: UIViewController {
    ...
    func tap(cell_No : Int){
        ...
        if chousingKoma != 0 && count < 3 && totalCost + komaCostIndex[String(chousingKoma)]! <= maxCost{
            ...
            if impossibleCell != cell_No && boardMatrix [(cell_No-1) / 3][(cell_No-1) % 3] != 0{ //未処理ーーーーーーーーーーー
                var soundIdRing:SystemSoundID = 0
    ...
    @IBAction func chose5(){
        protUpClear()
        komaButtns5.backgroundColor = UIColor.red
        chouse(komaNo: 12)
    }
    @IBAction func chose6(){
        protUpClear()
        komaButtns6.backgroundColor = UIColor.red
        chouse(komaNo: 14)
    }
    @IBAction func chose7(){
        protUpClear()
        komaButtns7.backgroundColor = UIColor.red
        chouse(komaNo: 16)
    }
